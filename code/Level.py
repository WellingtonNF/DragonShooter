        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                if event.type == EVENT_ENEMY:
                    choice = random.choice(('Enemy1', 'Enemy2'))  # faz uma escolha aleatoria entre os dois
                    self.entity_list.append(EntityFactory.get_entity(choice))
                if event.type == EVENT_TIMEOUT:
                    self.timeout -= TIMEOUT_STEP  # O TIMEOUT_STEP É O CONTADOR DE TEMPO, ele que vai decrementar
                    if self.timeout == 0:
                        for ent in self.entity_list:
